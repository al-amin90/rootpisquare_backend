...
const approveTenant = async (subdomain: string) => {
  ...
  if (!tenantRequest) throw new AppError(404, "Tenant request not found");

  if (tenantRequest.status === "approved")
    throw new AppError(404, "Already approved");
...
const loginUser = async (payload: LoginBody) => {
  ...
  if (dbManager.tenancyType === "single") {
    ...
    throw new AppError(404, "Unknown tenancy");
  }
};
...
const handleSingleTenantLogin = async (email: string, password: string) => {
  ...
  if (!tenantConn) throw new AppError(500, "Single DB not available");
...
const handleMultiTenantLoginAutoDetect = async (
  ...
  if (!envEmail || !envPassword) {
    throw new AppError(404, "Admin credentials not configured");
  }
  ...
  if (!isTenantExist) {
    throw new AppError(401, "This Shop is Not Exist");
  }

  if (!isTenantExist?.isActive) {
    throw new AppError(404, "This Shop is Not Active");
  }
